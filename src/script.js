...
L.tileLayer("https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}.png", {
  attribution:
    '&copy; <a href="https://carto.com/attributions">CARTO</a> | <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors',
}).addTo(map);
  ...
  countdownInterval = setInterval(() => {
    if (remainingTime <= 0) {
      ...
      document.getElementById(
        "next-update"
      ).textContent = `Next update in: ${remainingTime} seconds`;
      remainingTime--;
    }
        ...
        data.clients.pilots.forEach((pilot) => {
          ...
          if (markers[pilot.callsign]) {
            ...
            if (!currentPosition.equals(newPosition)) {
              ...
              var rotationAngle =
                Math.atan2(
                  newPosition.lng - currentPosition.lng,
                  newPosition.lat - currentPosition.lat
                ) *
                (180 / Math.PI);
              ...
              if (previousPosition) {
                var line = L.polyline([previousPosition, newPosition], {
                  color: "blue",
                }).addTo(map);
                marker.setIcon(
                  L.divIcon({
